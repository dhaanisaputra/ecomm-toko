                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key="{{ $item['product_id'] }}">
                                    ...
                                    <td class="py-4">{{ Number::currency($item['unit_amount'], 'IDR', 'id') }}</td>
                                    <td class="py-4">
                                        <div class="flex items-center">
                                    ...
                                    <td class="py-4">{{ Number::currency($item['total_amount'], 'IDR', 'id') }}</td>
                                    <td><button wire:click='removeCartItem({{ $item['product_id'] }})'
                                            ...
                                            <span wire:loading.remove
                                                wire:target='removeCartItem({{ $item['product_id'] }})'>Remove</span>
                                            <span wire:loading
                                                wire:target='removeCartItem({{ $item['product_id'] }})'>Removing...</span>
                                        </button>
                                    </td>
                                </tr>
                            @empty
            ...
            <div class="md:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span class="font-semibold">{{ Number::currency($grand_total, 'IDR', 'id') }}</span>
                    </div>
                    @if ($cart_items)
                        <a href="/checkout"
